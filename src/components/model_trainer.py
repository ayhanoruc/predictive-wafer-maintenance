...
from src.utility.metrics.classification_metrics import (
    cost_function,
...
class ModelTrainerComponent:
    ...
    def __init__(
        ...
        self.best_model_params = {"scale_pos_weight": 30, "reg_alpha": 0.1}
    ...
    def objective(self, trial):
        ...
        threshold = trial.suggest_float(
            "threshold", 0.005, 0.4, step=0.005
    ...
    @handle_exceptions
    def perform_hyperparams_opt(
        self,
    ):
        self.log_writer.handle_logging(
            "Hyperparams optimization initialized! Cost is being minimized!"
        )

        study = optuna.create_study(direction="minimize")
        study.optimize(
            self.objective, n_trials=5
        )  # >>> change n_trials to 100, its set to 5 for trial purpose
        ...
        self.log_writer.handle_logging(
            "The Hyperparams optimization study completed succesfully!"
        )
    ...
    def eval_model(self) -> tuple:
        ...
        f1_score, roc_auc_score, test_cost = get_classification_metrics(
            self.y_test, y_test_pred
        )

        self.log_writer.handle_logging("MODEL Trained/Tested Succesfully.")
        self.log_writer.handle_logging(
            "TESTING : F1-score: {f1_score}, ROC AUC: {roc_auc_score}, Cost: {test_cost} "
        )
    ...
    def run_model_trainer(
        ...
        self.log_writer.handle_logging(
            "Training & testing numpy arrays are loaded succesfully!"
        )
        ...
        print(
            f"roc_auc_score:{roc_auc_score} vs expected_roc_auc_score:{self.model_trainer_config.expected_roc_auc_score}"
        )
        print(
            f"f1_score:{f1_score} vs expected_f1_score:{self.model_trainer_config.expected_f1_score}"
        )
        ...
        if f1_score < self.model_trainer_config.expected_f1_score:
            ...
            raise Exception(
                "expected cost_score is not satisfied, try to do more Experimentation"
            )
        ...
        if roc_auc_score < self.model_trainer_config.expected_roc_auc_score:
            ...
            raise Exception(
                "expected roc_auc_score is not satisfied, try to do more Experimentation"
            )

        self.log_writer.handle_logging(
            "Calculated related metrics succesfully! Model is satistactory and is being saved!"
        )
        # if no exceptions raised -> save the model
        model_file_path = (
            self.model_trainer_config.model_trainer_trained_model_file_path
        )
        ...
        self.log_writer.handle_logging(
            f"ready to predict model is generated & saved succesfully!!! send to {model_file_path}"
        )

        classification_metrics_artifact = ClassificationMetricsArtifact(
            f1_score=f1_score, roc_auc_score=roc_auc_score, cost_score=test_cost
        )
