...
class TrainingPipeline:
    ...
    @handle_exceptions
    def start_component(
        self, config_class, component_class, run_method: str, *args, **kwargs
    ) -> object:
        """
        ...
        self.log_writer.handle_logging(
            f"{component.__class__} is succesfully initialized"
        )
    ...
    def start_data_ingestion(self) -> DataIngestionArtifact:
        return self.start_component(
            DataIngestionConfig, DataIngestionComponent, "run_data_ingestion"
        )
    ...
    def start_model_pusher(
        self, model_evaluator_artifact: ModelEvaluationArtifact
    ) -> ModelPusherArtifact:
        return self.start_component(
            ModelPusherConfig,
            ModelPusherComponent,
            "run_model_pusher",
    ...
    def run_training_pipeline(self, is_manual_ingestion=False):
        ...
        """
        self.log_writer.handle_logging(
            "-------------ENTERED TRAINING PIPELINE STAGE------------"
        )
        ...
        model_evaluator_artifact: ModelEvaluationArtifact = self.start_model_evaluation(
            data_validation_artifact, model_trainer_artifact
        )
        if not model_evaluator_artifact.is_model_accepted:
            self.log_writer.handle_logging(
                "Current trained model did not meet minimum accuracy requirements."
            )
            print(
                "Current Trained Model is not improved or couldn satisfied the min accuracy"
            )
            return False
